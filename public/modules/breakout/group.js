modules["breakout/group"] = class {
  ...
  js = async (frame, extra = {}) => {
    ...
    groupName.addEventListener("focusout", async () => {
      ...
      let oldName = this.group.name ?? "Untitled Team";
...
modules["modals/lesson/breakout/group/welcome"] = class {
  ...
  css = {
    ...
    ".brgWelcomeGroupMembersHolder": `width: min-content; min-width: 100%; margin: 8px 0`,
    ...
    ".brgWelcomeGroupMember": `--themeColor: var(--theme); --shadow: var(--lightShadow); position: relative; box-sizing: border-box; display: flex; max-width: 100%; padding: 8px 16px 8px 8px; background: var(--pageColor); border-radius: 18px; justify-content: center; align-items: center; transition: .2s`,
    ...
    ".brgWelcomeGroupMemberCursor": `flex-shrink: 0; position: relative; box-sizing: border-box; width: 32px; height: 32px; margin-right: 6px; background: var(--themeColor); border: solid 4px var(--pageColor); border-radius: 10px 16px 16px; transition: .2s`,
    ".brgWelcomeGroupMemberCursor:before": `content: ""; position: absolute; width: calc(100% + 6px); height: calc(100% + 6px); left: -3px; top: -3px; border-radius: inherit; contain: strict; box-shadow: 0 0 6px rgb(0 0 0 / 25%)`,
    ".brgWelcomeGroupMemberText": `font-size: 18px; font-weight: 600; white-space: nowrap; overflow-x: hidden; text-overflow: ellipsis; transition: .2s`,
