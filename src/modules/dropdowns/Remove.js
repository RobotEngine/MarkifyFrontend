      ...
      if (code == 200) {
        ...
        if (extra.parent != null) {
          if (option == "deletelesson") {
            ...
            if (folder != null) {
              let parentFolder = parent.folders[folder.render.parent];
              if (parentFolder != null && parentFolder.folders != null) {
                let folderIndex = parentFolder.folders.indexOf(extra.folderID);
                if (folderIndex > -1) {
                  parentFolder.folders.splice(folderIndex, 1);
                }
              }
              parent.removeFolder(folder);
              if (parent.sort == extra.folderID) {
                if (parentFolder != null && parentFolder.element != null) {
                  parentFolder.element.setAttribute("selected", "");
                  parent.changeSort(parentFolder.render._id);
                } else {
                  let recentSort = parent.sidebar.querySelector('.dSidebarSort[sort="recent"]');
                  recentSort.setAttribute("selected", "");
                  parent.changeSort("recent");
                }
              }
            }
          }
