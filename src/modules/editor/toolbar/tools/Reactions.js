...
class ReactionsDropdown {
  ...
  async js(frame, { parent }) {
    ...
    let toolbar = parent.toolbar;
    let editor = parent.editor;

    let cache = toolbar.reactionsCache;
    ...
    removeReactionButton.addEventListener("click", async () => {
      ...
      await sendRequest("DELETE", path, null, { session: editor.session });
    ...
    let updateReactionView = (noRefresh) => {
      ...
      if (editor.self.access > 3 && editor.utils.isLocked(toolbar.getPreferenceTool()) != true) {
    ...
    cache.function = (body) => {
      ...
      if (Object.keys(toolbar.reactionsCache.reactions).length > 0) {
        toolbar.selection.updateActionBar();
      } else {
        toolbar.selection.closeActionFrame();
      }
    }
