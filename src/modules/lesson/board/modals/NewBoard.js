...
import uploadIcon from "@assets/lesson/new/upload.svg?raw";

export class BlankPageModal {
  ppi = 96;
...
export class Frame {
  ...
  <div class="lessonCreationHolder">
    <div class="lessonBlankHolder">
      <button class="lessonBlank border" style="--themeColor: var(--gray)" title="Create a fresh blank page."><div image>${blankIcon}</div><div text>Blank Page</div></button>
      <button class="lessonFreeboard border" style="--themeColor: var(--purple)" title="Create an unlimited canvas for whiteboarding."><div image>${freeboardIcon}</div><div text>Freeboard</div></button>
    </div>
    <button class="lessonUpload border" style="--themeColor: var(--secondary)" title="Upload an existing PDF to markup."><div image>${uploadIcon}</div><div text>Upload PDF</div></button>
  </div>`;
  css = {
  ...
  js(frame, extra) {
    ...
    if (closeButton.hasAttribute("listener") == false) {
      closeButton.setAttribute("listener", "");
