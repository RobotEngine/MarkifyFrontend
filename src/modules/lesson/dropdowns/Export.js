import { assetURL, app, getEpoch, sendRequest } from "@/crucial";

import pagesIcon from "@assets/lesson/export/pages.svg?raw";
import boardIcon from "@assets/lesson/export/board.svg?raw";
import selectedIcon from "@assets/lesson/export/selected.svg?raw";
...
export class Frame {
  ...
  js(frame, extra) {
    ...
    if (type == "download") {
      ...
      frame.innerHTML = `
      <button class="eFileExportOption" option="pages" title="Print each page separately (RECOMENDED)."><div image>${pagesIcon}</div><div class="eFileExportInfo"><div class="eFileExportTitle">Just the <b>Pages</b></div><div class="eFileExportDesc">Print each page separately (RECOMENDED).</div></div></button>
      <button class="eFileExportOption" option="board" title="Print the entire board as a single page."><div image>${boardIcon}</div><div class="eFileExportInfo"><div class="eFileExportTitle">Entire <b>Document</b></div><div class="eFileExportDesc">Print the entire board as single page.</div></div></button>
      <button class="eFileExportOption" option="selected" title="Print only the selected elements."><div image>${selectedIcon}</div><div class="eFileExportInfo"><div class="eFileExportTitle">Selected <b>Elements</b></div><div class="eFileExportDesc">Print only the selected elements.</div></div></button>
      `;
    }
    ...
    let runExport = async (method) => {
      ...
      let exportAlert = await editor.openAlert("info", "<b>Exporting Lesson</b><div>Preparing export...</div>", { time: "never" });
      ...
      if (editor.exportStatus.exportAlert != null) {
        ...
        editor.closeAlert(editor.exportStatus.exportAlert);
      }
    ...
    editor.pipeline.subscribe("exportDropdownUpdate", "exportstatus", async (body) => {
      ...
      exportStatus.exportAlertTimeout = setTimeout(() => {
        editor.closeAlert(exportStatus.exportAlert);
      }, 60000);
