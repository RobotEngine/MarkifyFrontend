...
export class Frame {
  ...
  js(frame, extra) {
    ...
    if (this.modal.closeButton != null && this.modal.closeButton.hasAttribute("listen") == false) {
      this.modal.closeButton.setAttribute("listen", "");
      this.modal.closeButton.addEventListener("click", () => { this.updateSeenTutorial(); });
    }
    ...
    let updatePage = async () => {
      ...
      if (info == null) {
        this.close();
        return this.updateSeenTutorial();
      }
